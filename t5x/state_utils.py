...
def intersect_state(
    ...
    intersect_state_dict_flat = flatten_state_dict(
        intersect_state_dict, keep_empty_nodes=True
    )
...
def merge_state(
    state_dict: Mapping[str, Any], from_scratch_state: Mapping[str, Any]
) -> Mapping[str, Any]:
    ...
    """
    state_dict_flat = flatten_state_dict(state_dict, keep_empty_nodes=True)
    from_scratch_state_flat = flatten_state_dict(
        from_scratch_state, keep_empty_nodes=True
    )
    ...
    *,
    ...
    for key in set(flat_opt) - set(result):
        if key in explicitly_skipped_keys:
            ...
            logging.warning(
                "Skipping key=%s which did not match assignment map or checkpoint.", key
            )
...
def str_flatten_dict(state_dict):
    return (
        "{\n"
        + ",\n".join(
            ["{}: {}".format(k, v) for k, v in flatten_state_dict(state_dict).items()]
        )
        + "\n}"
    )


def str_set(state_set):
    return "{\n" + ",\n".join(["{}".format(k) for k in state_set]) + "\n}"
