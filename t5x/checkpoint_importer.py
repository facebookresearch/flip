...
class LazyArray(abc.ABC):
    ...
    def __init__(
        self, shape: Sequence[int], dtype: jnp.dtype, get_fn: Callable[[], np.ndarray]
    ):
    ...
    def astype(self, dtype: np.dtype) -> "LazyArray":
        return type(self)(
            self._shape, dtype, self._get_fn
...
def layernorms(opts, key, val, encdec, blocknum, lyrnum, slot):
    ...
    if encdec == "transformer":
        layernorm_type = ["pre_self_attention_layer_norm", "pre_mlp_layer_norm"][lyrnum]
...
@t5_importer.add(r"(encoder|decoder|transformer)/(?:final_layer|rms)_norm/scale(\w*)")
