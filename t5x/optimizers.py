...
class OptaxStatePartitionRules:
    ...
    _RULES = {
        # Leaf Optax States:
        optax.AddNoiseState: lambda state, params_axes: optax.AddNoiseState(
            count=None, rng_key=None
        ),
        optax.DifferentiallyPrivateAggregateState: lambda state, params_axes: optax.DifferentiallyPrivateAggregateState(
            rng_key=None
        ),
        optax.EmaState: lambda state, params_axes: optax.EmaState(
        ...
        optax.ScaleByAdamState: lambda state, params_axes: optax.ScaleByAdamState(
            count=None, mu=params_axes, nu=params_axes
        ),
        # utils.adamw.ScaleByAdaRowsState:
        #     utils.adamw.PartitionRuleScaleByAdaRowsState,
        optax.ScaleByBeliefState: lambda state, params_axes: optax.ScaleByBeliefState(
            count=None, mu=params_axes, nu=params_axes
        ),
        ...
        optax.ScaleByRStdDevState: lambda state, params_axes: optax.ScaleByRStdDevState(
            mu=params_axes, nu=params_axes
        ),
        optax.ScaleBySM3State: lambda state, params_axes: optax.ScaleBySM3State(
            mu=params_axes, nu=params_axes
        ),
        optax.ScaleByTrustRatioState: lambda state, params_axes: optax.ScaleByTrustRatioState(),
        optax.ScaleByScheduleState: lambda state, params_axes: optax.ScaleByScheduleState(
            count=None
        ),
        optax.ScaleByFromageState: lambda state, params_axes: optax.ScaleByFromageState(
            count=None
        ),
        ...
        optax.InjectHyperparamsState: lambda state, params_axes: optax.InjectHyperparamsState(
            count=None,
        ...
        optax.MultiStepsState: lambda state, params_axes: optax.MultiStepsState(
            mini_step=None,
            gradient_step=None,
        ...
        optax.ApplyIfFiniteState: lambda state, params_axes: optax.ApplyIfFiniteState(
        ...
        optax.MaybeUpdateState: lambda state, params_axes: optax.MaybeUpdateState(
            inner_state=OptaxStatePartitionRules.derive_optax_logical_axes(
                state.inner_state, params_axes
            ),
            step=None,
        ),
        optax.MultiTransformState: lambda state, params_axes: optax.MultiTransformState(
...
class OptaxWrapper(OptimizerDef):
    ...
    def init_state(self, params):
        ...
        """
        state = OptimizerState(step=0, param_states=self.optax_optimizer.init(params))
        return state
    ...
    def derive_logical_axes(self, optimizer, param_logical_axes):
        ...
        """
        optimizer_logical_axes = jax.tree_map(lambda x: None, optimizer.state_dict())
        ...
        optimizer_logical_axes["state"]["param_states"] = serialization.to_state_dict(
            optax_state_axes
        )
...
def _tree_of_paths(tree):
    ...
    path_tree = traverse_util.unflatten_dict({k: "/".join(k) for k in flat_tree.keys()})
    if is_frozen:
...
class MultiOptimizer(OptimizerDef):
    ...
    def derive_logical_axes(self, optimizer, param_logical_axes):
        ...
        param_states = jax.tree_map(lambda x: _Marker(), optimizer.state.param_states)
        for focus, opt_def in zip(self.traversals, self.sub_optimizers):
