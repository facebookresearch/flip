...
def _choose_chunk_shape(write_shape: Sequence[int], target_elements: int) -> List[int]:
...
class SaveStateTransformationFn(typing_extensions.Protocol):
    def __call__(
        self, state_dict: PyTreeDef, parameter_infos: PyTreeDef
...
class Checkpointer(object):
        ...
        *,
        ...
        if checkpoints_dir.startswith("gs://"):
            ...
            checkpoints_dir = os.path.join(
                f"gs://kmh-gcp/checkpoints/flax/t5x_tmp_{timestamp}"
            )
        ...
        self._dataset_ckpt_name = (
            f"train_ds-" f"{data_layout.shard_id:03}-of-{data_layout.num_shards:03}"
        )
    ...
    def _get_parameter_infos(self):
        ...
        def _get_param_info(name: str, arr: Any, axes: partitioning.PartitionSpec):
            ...
            if self._use_gda and isinstance(arr, gda_lib.GlobalDeviceArray):
                local_chunk_info = None
                metadata = gda_serialization._get_metadata(
                    arr
    ...
    def save(
        ...
        *,
        ...
        # Block until complete on all hosts.
        multihost_utils.sync_global_devices(f"checkpointer:write_complete:{final_dir}")
    ...
    def _write_state_to_tensorstore(
        ...
        ) = self._transform_state_and_infos(
            train_state.state_dict(), self._parameter_infos, state_transformation_fns
        )
...
class SaveBestCheckpointer(Checkpointer):
    ...
    def _populate_metrics_for_steps(self, steps: Iterable[int]) -> Mapping[int, float]:
        ...
        for subdir in io_wrapper.GetLogdirSubdirectories(self.checkpoints_dir):
            ...
            if (
                not self._metric_run
                and not self._try_fill_metric_run_and_tag_names((rpath,))
            ) or self._metric_run != rpath:
            ...
            for event in loader.Load():
                ...
                if (
                    ...
                    or event.summary.value[0].tag != self._metric_tag
                ):
                    continue
    ...
    def _filter_out_force_keep_period_steps(self, existing_steps):
        ...
        existing_steps = [s for s in existing_steps if s % self._force_keep_period != 0]
...
async def _read_ts(
    ...
    if ("dtype" in tmp_ts_spec_dict and tmp_ts_spec_dict["dtype"] == "uint16") or (
...
def load_t5x_checkpoint(
    ...
    state_dict = jax.tree_multimap(
        functools.partial(
            _create_lazy_awaitable_array, ckpt_path=path, restore_dtype=restore_dtype
        ),
        param_infos,
        ckpt_optimizer_state_with_specs,
    )
