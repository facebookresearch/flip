...
def log_model_info(
    ...
    with contextlib.ExitStack() as stack:
        ...
        mesh_shape = (
            partitioner._local_chunker.global_mesh.shape
        )  # e.g., OrderedDict([('data', 2), ('model', 4)])
        ...
        def _log_variable(
            ...
            logging.info(
                "{:108s} {:>16s} {:48s} {}".format(
                    name, arr_size, str(shape_str), str(mesh_axes)
                )
            )
        ...
        _log_info_and_write_to_file(
            writer, "Total number of parameters (1e6): %.6f", total_num_params / 1e6
        )
        _log_info_and_write_to_file(
            writer,
            "Total number of image encoder parameters (1e6): %.6f",
            image_num_params / 1e6,
        )
        _log_info_and_write_to_file(
            writer,
            "Total number of txt encoder parameters (1e6): %.6f",
            txt_num_params / 1e6,
        )
        _log_info_and_write_to_file(
            writer,
            "Total number of parameter_states (1e6): %.6f",
        ...
        _log_info_and_write_to_file(
            writer,
            "Estimated model memory with all float32 (G): %.6f",
            expected_memory / 1024 / 1024 / 1024,
        )


def log_state_info(
    real_train_state: train_state_lib.TrainState,
):
    """Log the variable shapes information and optionally write it to a file."""
    # Only write logs on host 0.
    if jax.process_index() != 0:
    ...
    def parse_sharding(x):
        return np.array(
            [
                i.chunks[0] if hasattr(i, "chunks") else 1
                for i in x.sharding_spec.sharding
            ]
            if hasattr(x, "sharding_spec")
            else [
                1,
            ]
        )
    ...
    bytes = jax.tree_map(
        lambda x: 4 if x == jnp.float32 else (2 if x == jnp.bfloat16 else 4), types
    )
    ...
    logging.info(
        "Est. memory of parameters per device (G): %.6f",
        total_mem_params / 1024 / 1024 / 1024,
    )
    logging.info(
        "Est. memory of parameter_states per device (G): %.6f",
        total_mem_states / 1024 / 1024 / 1024,
    )
    logging.info(
        "Est. memory of both per device (G): %.6f",
        (total_mem_params + total_mem_states) / 1024 / 1024 / 1024,
    )
